...
function ScrollSwipe(opts) {
  ...
  if (!this.scrollMinDistance || this.scrollMinDistance < 0) {
    this.scrollMinDistance = 0
  }
...
ScrollSwipe.prototype.onWheel = function onWheel(e) {
    ...
    const x = normalizeWheel(e).pixelX;
    const y = normalizeWheel(e).pixelY;

    this.addXScroll(x);

    if (Math.abs(y) > this.scrollMinDistance) {
      this.addYScroll(y);
    }
